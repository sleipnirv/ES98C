...
def _parse_ocr_float(text: str) -> float:
    """Parse an OCR'd axis-label string into a float.

    The VLM (qwen3-vl) sometimes returns decimals with a comma separator
    (``-0,8``) or a unicode minus sign (``− 0.8``). Normalize these before
    ``float()`` so a single bad label doesn't kill the whole panel.
    """
    s = str(text).strip()
...
def sort_and_check_labels(
    ...
    ocr_values = {
        k: _parse_ocr_float(v) for k, v in axis_label_texts.items() if "plot_area" not in k
    }
...
def convert_data_points(conversions, base_output_dir: str, img: str, label_coordinates: dict):
    ...
    for series_index, lineseries in enumerate(all_lineseries):
        # Apply the fitted affine transform value = slope * pixel + intercept
        # directly. The regression already encodes the axis direction and
        # absolute scale, so no manual flip or offset is needed.
        converted_points = [
            {"x": convertx(point["x"]), "y": converty(point["y"])}
